...
| 7 | [Prove an IAM Permissions Boundary](./systems/iam-permissions-boundary-proof/) | Cloud security and authorization evidence | A permissions boundary tested rather than assumed: eight decisions frozen before the simulator ran, an unbounded control condition first returning allowed for the disputed CreatePolicyVersion, then refused with AllowedByPermissionsBoundary false once the ceiling was attached. Explicit deny, implicit deny, boundary refusal and mutual allowance kept as four separate outcomes, scoring 8 of 8, with teardown graded by fresh NoSuchEntity reads rather than by the delete commands |

**7 of 100 shipped.** The rest arrive one live build at a time.
